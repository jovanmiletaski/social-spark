...
const socialPlatforms = [
  {
    ...
    followers: "2343.4k",
    change: "+2,560",
    bgClass: "bg-foreground text-card",
  },
  {
    name: "Instagram",
    icon: (
    ...
    <>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {socialPlatforms.map((platform) => (
          <Card key={platform.name} className={`${platform.bgClass} border-0 group cursor-pointer hover:scale-[1.02] transition-transform`}>
            <CardContent className="p-4">
              ...
              <div className="flex items-center gap-1 text-xs mt-1 opacity-80">
                ...
                {platform.change}
                <span className="hidden sm:inline">from previous month</span>
              </div>
              <Button
      ...
      <AIInsightsModal open={aiModalOpen} onOpenChange={setAiModalOpen} title={selectedPlatform} />
    </>
  )
}
